...
for (const r of RARITY_ORDER) {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    if (cur.w * cur.h > prev.w * prev.h && cur.prices.common < prev.prices.common) {
      bad(`${rarityLabel(r)}：${cur.name} 更占格却比 ${prev.name} 便宜`);
    }
  }
}
  ...
  for (const a of rows) {
    for (const b of rows) {
      if (rank[a.rarity] > rank[b.rarity] && a.price <= b.price) {
        bad(`${area} 格：${a.name}(${rarityLabel(a.rarity)}) 不比 ${b.name}(${rarityLabel(b.rarity)}) 贵`);
      }
    }
  }
}
...
Object.keys(COOK_UNLOCK_AT).forEach((lv) => {
...
for (const m of MARKETS) {
  ...
  const rar = pool.map((id) => rarityLabel(RECIPES.find((r) => r.id === id)!.rarity));
