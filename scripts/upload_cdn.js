#!/usr/bin/env node
/**
 * 捡菜市集 CDN 资源增量上传（腾讯云 COS / CloudBase 存储桶）
 *
...
const CDN_PREFIXES = cfg.cdnPrefixes || [];
...
function isCdnRemote(remote) {
  if (!CDN_PREFIXES.length) return true;
  return CDN_PREFIXES.some(prefix => remote === prefix || remote.startsWith(prefix));
}
...
function walkDir(dir, remotePrefix) {
  ...
  for (const item of fs.readdirSync(dir)) {
    ...
    else if (isCdnRemote(remote)) out.push({ local: full, remote, size: stat.size });
  }
  return out;
}
...
function syncPackIgnore(remotePaths) {
  ...
  const globs = [
    ...
    { type: 'glob', value: 'subpkg_images/stall_rummage_*.jpg' },
  ...
  project.packOptions = pack;
  fs.writeFileSync(configPath, `${JSON.stringify(project, null, 2)}\n`);
}

async function main() {
  console.log('=== 捡菜市集 CDN 资源上传（腾讯云 COS / CloudBase）===');
