...
// ======== URL / Blob ========
// Pixi 路径解析会 `new URL(src, base)`。微信里没有 URL 构造函数；
// 模拟器若用对象覆盖原生 URL，同样会炸成 "URL is not a constructor"。
function MiniURL(url, base) {
  if (!(this instanceof MiniURL)) return new MiniURL(url, base);
  var href = String(url == null ? '' : url);
  if (base && !/^[a-zA-Z][a-zA-Z\d+\-.]*:/.test(href)) {
    var b = String(base);
    if (href.charAt(0) === '/') {
      var origin = b.match(/^[a-zA-Z][a-zA-Z\d+\-.]*:\/\/[^/]*/);
      href = (origin ? origin[0] : '') + href;
    } else {
      var cut = b.lastIndexOf('/');
      href = (cut >= 0 ? b.slice(0, cut + 1) : b) + href;
    }
  }
  this.href = href;
  this.pathname = href;
...
MiniURL.createObjectURL = function() { return ''; };
MiniURL.revokeObjectURL = function() {};
const _nativeURL = typeof URL === 'function' ? URL : null;
const _URL = _nativeURL || MiniURL;
function _Blob() {}
...
if (isDevtools) {
  ...
  for (const key in _allGlobals) {
    ...
    if (key === 'URL' && typeof URL === 'function') continue;
    try {
  ...
  var _forceOverwrite = ['addEventListener', 'removeEventListener', 'URL'];
